...
# edit by Taizhou
def get_pos_list(path_to_pos2id):
    """
    get a list of pos and pos one hot list
    :param path_to_pos2id: path to pos2id.txt
    :return: a list of pos and their corresponding one hot representation
    """

    pos_list = []
    pos_id = []

    def read_file(path_to_pos2id):
        with open(path_to_pos2id, 'r') as f:
    ...
    if not os.path.exists(path_to_pos2id):
        print("Missing file pos2id.txt")
        exit(-1)

    file_context = read_file(path_to_pos2id)
    for i, line in enumerate(file_context):
        pos_list.append(line.split('\t')[0])
        pos_id.append(i)

    one_hot_list = convert_to_one_hot(pos_id)

    return pos_list, one_hot_list

# edit by Taizhou
def get_pos_info(tokens, pos_list, pos_one_hot_list):
    """
    return pos information for a input tokens
    :param tokens: input tokens
    :param pos_list: list of pos ['CC', 'CD', 'DT', ...]
    :param pos_one_hot_list: a 2d array of one hot vector for each pos in the pos list
    :return: pos, pos_id, pos_one_hot
    """
    pos = []
    pos_ids = []
    pos_one_hot = None
    ...
    for pos_tag_ in pos_tag:
        pos.append(pos_tag_[1])
        pos_id_ = 0
        try:
            pos_id_ = pos_list.index(pos_tag_[1]) + 1
        except:
            print(pos_tag_)
            print('POS index error!')
            pass

        pos_ids.append(pos_id_)

    pos_one_hot = pos_one_hot_list[[i-1 for i in pos_ids]]

    return pos, pos_ids, pos_one_hot
...
def convert_single_example(ex_index, example, label_list, max_seq_length, tokenizer, output_dir, mode, pos_list, pos_one_hot_list):
    """
    ...
    if ex_index < 5:
        ...
        # edit by Taizhou
        logger.info("pos: %s" % " ".join([str(x) for x in pos]))
        logger.info("pos_id: %s" % " ".join([str(x) for x in pos_ids]))
        logger.info("pos_one_hot: %s" % " ".join([str(x) for x in pos_one_hot]))
...
def filed_based_convert_examples_to_features(
        examples, label_list, max_seq_length, tokenizer, output_file, output_dir, pos_list, pos_one_hot_list, mode=None):
    """
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        # 对于每一个训练样本,
        feature = convert_single_example(ex_index, example, label_list, max_seq_length, tokenizer, output_dir, mode, pos_list, pos_one_hot_list)
        ...
        # edit by Taizhou
        def create_floats_feature(values):
        ...
        features["pos_one_hot"] = create_floats_feature(feature.pos_one_hot)
...
def file_based_input_fn_builder(input_file, seq_length, is_training, drop_remainder, pos_list_length):
    name_to_features = {
    ...
    # # edit by Taizhou
    # name_to_features_pos = {
    #     "pos_one_hot": tf.FixedLenFeature([seq_length], tf.float32)
    # }
...
def model_fn_builder(bert_config, num_labels, init_checkpoint, learning_rate,
    ...
    def model_fn(features, labels, mode, params):
        ...
        # pos_one_hot = features["pos_one_hot"]
        ...
        total_loss, logits, trans, pred_ids = create_model(
            ...
            num_labels, False, args.dropout_rate, args.lstm_size, args.cell, args.num_layers, args.use_pos, args.crf_only)
...
def train(args):
    ...
    if args.do_train and args.do_eval:
        ...
        if not os.path.exists(train_file):
            filed_based_convert_examples_to_features(
                train_examples, label_list, args.max_seq_length, tokenizer, train_file, args.output_dir, pos_list=pos_list, pos_one_hot_list=pos_one_hot_list)
        ...
        if not os.path.exists(eval_file):
            filed_based_convert_examples_to_features(
                eval_examples, label_list, args.max_seq_length, tokenizer, eval_file, args.output_dir, pos_list=pos_list, pos_one_hot_list=pos_one_hot_list)
    ...
    if args.do_predict:
        ...
        filed_based_convert_examples_to_features(predict_examples, label_list,
                                                 ...
                                                 predict_file, args.output_dir, mode="test", pos_list=pos_list, pos_one_hot_list=pos_one_hot_list)
        ...
        print('predict time: {}'.format((time.time() - tic)))
