...
class InputFeatures(object):
    ...
    def __init__(self, input_ids, input_mask, segment_ids, label_ids, pos=None, pos_ids=None, pos_one_hot=None):
...
def create_model(bert_config, is_training, input_ids, input_mask,
                 segment_ids, labels, pos_ids, num_labels, use_one_hot_embeddings,
                 dropout_rate=1.0, lstm_size=1, cell='lstm', num_layers=1, use_pos_feature=False, crf_only=False):
    """
    创建X模型
    ...
    # edited by Taizhou
    if use_pos_feature:
        print('Using POS feature...')
        one_hot = tf.one_hot(pos_ids, depth=35)
        embedding_one_hot = tf.concat([embedding, one_hot], axis=2)
        print(one_hot)
        print(embedding)
        print(embedding_one_hot)
