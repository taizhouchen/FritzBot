    ...
    def blstm_layer(self, embedding_chars):
        ...
        with tf.variable_scope('rnn_layer'):
            ...
            if self.num_layers > 1:
                ...
                # edited bt Taizhou
                cell_fw = rnn.MultiRNNCell([self.get_a_cell(self.hidden_unit, self.dropout_rate) for _ in range(self.num_layers)], state_is_tuple=True)
                cell_bw = rnn.MultiRNNCell([self.get_a_cell(self.hidden_unit, self.dropout_rate) for _ in range(self.num_layers)], state_is_tuple=True)
